...
public class CoffeeController {
    ...
    @PutMapping("/{id}")
    public void updateCoffee(@RequestBody CoffeeDTO coffeeDTO) {
    ...
    public List<CoffeeDTO> filterCoffees(@RequestParam(name = "name") String name) {
        return coffeeService.filterCoffeesByName(name).stream()
                .map(coffeeMapper::coffeeToDTO).collect(Collectors.toList());
    }
